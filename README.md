...
The output image resolution can be set with `-r`/`--resolution` (default is 1920x1080), and the output file name with `-o`/`--output` (default is `output.png`; the extension sets the image format). Pass `-n`/`--no-show` to skip opening the image after saving:
```
