...
```
python average-colours.py [URL] [-r WIDTHxHEIGHT] [-o OUTPUT_FILE] [-n]
```
...
4. Draws one vertical line per frame colour, resizes to the requested resolution (default 1920x1080), saves to the requested file (default `output.png` in the repo root; extension determines format), and opens it in the default viewer unless `-n`/`--no-show` is given.
